...
var spine = (() => {
  ...
  var SpinePipe = class {
    ...
    static extension = {
      type: [
        import_pixi4.ExtensionType.WebGLPipes,
        import_pixi4.ExtensionType.WebGPUPipes,
        import_pixi4.ExtensionType.CanvasPipes
      ],
      name: "spine"
    };
    renderer;
      ...
      for (let i = 0, n = drawOrder.length; i < n; i++) {
        ...
        if (attachment instanceof RegionAttachment || attachment instanceof MeshAttachment) {
          ...
          batchableSpineSlot.setData(
            spine,
            cacheData,
            attachment.region?.texture.texture || import_pixi4.Texture.EMPTY,
            blendMode,
            roundPixels
          );
        ...
        if (containerAttachment) {
          ...
          (0, import_pixi4.collectAllRenderables)(container, instructionSet, this.renderer.renderPipes);
  ...
  import_pixi4.extensions.add(SpinePipe);
  ...
  var Spine = class _Spine extends import_pixi5.Container {
    // Pixi properties
    batched = true;
    buildId = 0;
    renderPipeId = "spine";
    ...
    _boundsDirty = true;
    _roundPixels;
    _bounds = new import_pixi5.Bounds();
    ...
    set autoUpdate(value) {
      if (value) {
        import_pixi5.Ticker.shared.add(this.internalUpdate, this);
        this.autoUpdateWarned = false;
      } else {
        import_pixi5.Ticker.shared.remove(this.internalUpdate, this);
      }
    ...
    internalUpdate(_deltaFrame, deltaSeconds) {
      this._updateState(deltaSeconds ?? import_pixi5.Ticker.shared.deltaMS / 1e3);
    }
    get bounds() {
    ...
    updateSlotObject(slotAttachment) {
      ...
      if (container.visible) {
        ...
        container.rotation = bone.getWorldRotationX() * import_pixi5.DEG_TO_RAD;
      }
    }
    /** @internal */
    ...
    removeSlotObject(slotOrContainer) {
      ...
      if (slotOrContainer instanceof import_pixi5.Container) {
        for (const i in this._slotsObject) {
    ...
    static from({ skeleton, atlas, scale = 1 }) {
      ...
      if (import_pixi5.Cache.has(cacheKey)) {
        return new _Spine(import_pixi5.Cache.get(cacheKey));
      }
      const skeletonAsset = import_pixi5.Assets.get(skeleton);
      const atlasAsset = import_pixi5.Assets.get(atlas);
      ...
      import_pixi5.Cache.set(cacheKey, skeletonData);
      return new _Spine({
        skeletonData
      });
    }
  ...
  var SpineDebugRenderer = class {
    ...
    registerSpine(spine) {
      ...
      const debugDisplayObjects = {
        parentDebugContainer: new import_pixi6.Container(),
        bones: new import_pixi6.Container(),
        skeletonXY: new import_pixi6.Graphics(),
        regionAttachmentsShape: new import_pixi6.Graphics(),
        meshTrianglesLine: new import_pixi6.Graphics(),
        meshHullLine: new import_pixi6.Graphics(),
        clippingPolygon: new import_pixi6.Graphics(),
        boundingBoxesRect: new import_pixi6.Graphics(),
        boundingBoxesCircle: new import_pixi6.Graphics(),
        boundingBoxesPolygon: new import_pixi6.Graphics(),
        pathsCurve: new import_pixi6.Graphics(),
        pathsLine: new import_pixi6.Graphics(),
        eventText: new import_pixi6.Container(),
        eventCallback: {
          event: (_, event) => {
            if (this.drawEvents) {
              const scale = Math.abs(spine.scale.x || spine.scale.y || 1);
              const text = new import_pixi6.Text({
    ...
    drawBonesFunc(spine, debugDisplayObjects, lineWidth, scale) {
      ...
      for (let i = 0, len = bones.length; i < len; i++) {
        ...
        const gp = new import_pixi6.Graphics();
